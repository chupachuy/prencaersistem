...
<head>
    ...
    <style>
        ...
        body {
            font-family: 'Helvetica', 'Arial', sans-serif; padding: 0 30px 40px 30px; color: #333;
        }
...
<div class="document" style="padding-top:10mm;">
    ...
    <?php if ($consentimiento['estado'] === 'Revocado'): ?>
        <div class="denegacion" style="text-align:center; margin-top:20px; padding:10px; border:2px solid #d00; font-size:12px;">
            ESTE DOCUMENTO HA SIDO REVOCADO POR EL PACIENTE
        </div>
    <?php endif; ?>
</div>
                    <div class="signature-date"><?php echo date('d/m/Y H:i', strtotime($firma['fecha_firma'])); ?></div>
                    <?php if (!empty($firma['geo_pais'])): ?>
                    <div class="signature-location">&#x1f4cd; <?php echo htmlspecialchars(trim($firma['geo_ciudad'] . ', ' . $firma['geo_region'] . ', ' . $firma['geo_pais'], ', ')); ?></div>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
...
<div class="no-print" style="text-align:center;margin:20px 0;"><a href="<?php echo Url::to('/consentimientos/pdf?id=' . $consentimiento['id']); ?>"
